...
function generatePDF(resume) {
    ...
    doc.addImage(aqui, 'JPEG', doc.internal.pageSize.width-MARGIN-30-2, MARGIN, 30, 30, 'monkey');

    // BASICS
    doc.setFontSize(22).setTextColor(255,0,0).setFont("times").setFontType("italic")
    .text(name, doc.internal.pageSize.width-MARGIN-30-10, MARGIN+15 ,null, null, 'right')
    doc.setTextColor(10,10,10).setFontSize(10)
    .text(label, doc.internal.pageSize.width-MARGIN-30-10, MARGIN+30, null, null, 'right')
    ...
    .text('About', MARGIN, newLineY(10))
    doc.line(20, cursorY+2, doc.internal.pageSize.width-MARGIN, cursorY+2) // horizontal line
    // .text(summary, MARGIN, 60)
    .text(doc.splitTextToSize(summary, doc.internal.pageSize.width-2*MARGIN), MARGIN, newLineY(10));


    // EDUCATION
    newLineY(20)
    doc.setFontSize(10).setFont("times").setFontType("italic")
    ...
    resume.education.map(x => {
        ...
        doc.setFontSize(10).setFont("times").setFontType("bold")
         .text(institution+ ' '+ area, MARGIN+10, newLineY(5))
         .setFontType("italic")
         .text(`${startDate} - ${endDate}`, doc.internal.pageSize.width-MARGIN, cursorY,null, null, 'right')    
    })    
    ...
    resume.skills.map(x => {
        ...
        doc.text( keywords.join(' | '), doc.internal.pageSize.width-MARGIN, cursorY,null, null, 'right')
    })    
    ...
    newLineY(10)
    doc.setFontType("italic")
    .text('Experience', MARGIN, newLineY())
    doc.line(20, cursorY+2, doc.internal.pageSize.width-MARGIN, cursorY+2); // horizontal line
    newLineY()

    // eslint-disable-next-line array-callback-return
    resume.works.map(x => {
        const { company, position, startDate, endDate='current', highlights, stack } = x    // website summary

        doc.setTextColor(10,10,10).setFontSize(10).setFont("times").setFontType("bold").text(company, MARGIN+10, newLineY(10))
        doc.setTextColor(175,10,10).setFontType("italic").text(position, MARGIN+130 , cursorY ) // newLineY()
        doc.setTextColor(10,10,10).text(`${startDate} - ${endDate}`, doc.internal.pageSize.width-MARGIN, cursorY,null, null, 'right')
        ...
        newLineY()
        doc.setFontSize(9).setTextColor(100,100,100).setFont("times").setFontType("bold")        
        .text( stack.join(' | '), doc.internal.pageSize.width-MARGIN, cursorY,null, null, 'right')
        newLineY()
...
function generateCoverPDF(resume){
    ...
    // BASICS
    doc.setFontSize(12).setFont("times").setFontType("italic")
    .text(doc.splitTextToSize(cover, doc.internal.pageSize.width-2*MARGIN), MARGIN, newLineY(30));
